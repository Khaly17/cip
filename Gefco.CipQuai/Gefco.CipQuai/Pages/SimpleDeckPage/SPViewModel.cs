...
namespace Gefco.CipQuai.SimpleDeckPage
{
    ...
    public class SPViewModel : ViewModelBaseExt
    {
        ...
        public string StartPageInvite => $"Veuillez indiquer l'agence de destination";
        public string NextButtonLabel { get; private set; } = "Suivant >";
        public string DestinationInviteLabel { get; private set; } = "Agence de destination";
        ...
        {
            foreach (var destination in destinations.Where(p => p.IsGefcoFrance || p.IsClient).OrderBy(p => p.Name))
            {
        ...
        {
            Reset();
            Declaration = new DeclarationSimplePlancher()
            {
        ...
        public DeclarationSimplePlancher Declaration { get; set; }
        private async Task<string> SavePictureAsync(string filePath, PictureType pictureType)
        {
            if (!filePath.IsNullOrWhiteSpace() && filePath != "Camera.svg")
            {
                if (!filePath.StartsWith("http"))
                {
                    var parameter = new Settings.UploadPictureParameter()
                                    {
                                        ...
                                        PictureType = Settings.UploadPictureType.SP
                                    };
                    ...
                    var result = await DataService.UploadPicSP(fileContent, Path.GetFileName(filePath), pictureType, Declaration.Id);
        ...
        public async Task EndDeclaration()
        {
            ...
            var res = await DataService.AddDeclarationRemorque(Declaration);
            if (res.IsSuccess ?? false)
            {
                App.Settings.AddDeclarationRemorques.RemoveWhere(p => p.Id == Declaration.Id);
