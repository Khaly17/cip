// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System;
using System.ComponentModel;
using System.Globalization;

namespace Gefco.CipQuai.ApiClient.Models
{
    /// <summary> The Enum7. </summary>
    public readonly partial struct Enum7 : IEquatable<Enum7>
    {
        private readonly int _value;

        /// <summary> Initializes a new instance of <see cref="Enum7"/>. </summary>
        public Enum7(int value)
        {
            _value = value;
        }

        private const int ZeroValue = 0;
        private const int OneValue = 1;
        private const int TwoValue = 2;
        private const int ThreeValue = 3;
        private const int FourValue = 4;
        private const int FiveValue = 5;
        private const int SixValue = 6;
        private const int SevenValue = 7;
        private const int EightValue = 8;
        private const int NineValue = 9;

        /// <summary> 0. </summary>
        public static Enum7 Zero { get; } = new Enum7(ZeroValue);
        /// <summary> 1. </summary>
        public static Enum7 One { get; } = new Enum7(OneValue);
        /// <summary> 2. </summary>
        public static Enum7 Two { get; } = new Enum7(TwoValue);
        /// <summary> 3. </summary>
        public static Enum7 Three { get; } = new Enum7(ThreeValue);
        /// <summary> 4. </summary>
        public static Enum7 Four { get; } = new Enum7(FourValue);
        /// <summary> 5. </summary>
        public static Enum7 Five { get; } = new Enum7(FiveValue);
        /// <summary> 6. </summary>
        public static Enum7 Six { get; } = new Enum7(SixValue);
        /// <summary> 7. </summary>
        public static Enum7 Seven { get; } = new Enum7(SevenValue);
        /// <summary> 8. </summary>
        public static Enum7 Eight { get; } = new Enum7(EightValue);
        /// <summary> 9. </summary>
        public static Enum7 Nine { get; } = new Enum7(NineValue);
        /// <summary> Determines if two <see cref="Enum7"/> values are the same. </summary>
        public static bool operator ==(Enum7 left, Enum7 right) => left.Equals(right);
        /// <summary> Determines if two <see cref="Enum7"/> values are not the same. </summary>
        public static bool operator !=(Enum7 left, Enum7 right) => !left.Equals(right);
        /// <summary> Converts a string to a <see cref="Enum7"/>. </summary>
        public static implicit operator Enum7(int value) => new Enum7(value);

        /// <inheritdoc />
        [EditorBrowsable(EditorBrowsableState.Never)]
        public override bool Equals(object obj) => obj is Enum7 other && Equals(other);
        /// <inheritdoc />
        public bool Equals(Enum7 other) => Equals(_value, other._value);

        /// <inheritdoc />
        [EditorBrowsable(EditorBrowsableState.Never)]
        public override int GetHashCode() => _value.GetHashCode();
        /// <inheritdoc />
        public override string ToString() => _value.ToString(CultureInfo.InvariantCulture);
    }
}
