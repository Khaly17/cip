...
{
    [Authorize]
    public class SimplesPlanchersController : Controller
    {
        private readonly ApplicationDbContext _ctx;
        public SimplesPlanchersController()
        {
        ...
        {
            var declaration = _ctx.DeclarationSimplePlanchers.SingleOrDefault(p => p.Id == id);
            var tempPictures = _ctx.TempPictures.Where(p => p.DeclarationBonnePratique_Id == id).ToList();
            var viewModel = new DetailSPViewModel(declaration, tempPictures);
