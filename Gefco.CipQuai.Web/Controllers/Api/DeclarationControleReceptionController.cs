...
{
    public class DeclarationControleReceptionController : ApiController
    {
        [Route(nameof(AddDeclarationControleReception))]
        public BooleanServiceResult AddDeclarationControleReception(string appVersion, string userId, DeclarationControleReception declaration)
        {
            using (var dal = new Dal())
            {
                ...
                var decl = dal.FindDeclarationControleReception(declaration.Id);
                if (decl != null)
                {
                ...
                decl = dal.ObjectContext.DeclarationControleReceptions.SingleOrDefault(p => p.Id == declaration.Id);
                if (decl != null)
                {
                    var exception = new DeclarationExistsException();
                    SimpleLogger.GetOne().Error(exception);
                ...
                try
                {
                    ...
                    //todo declaration.CurrentStatus = dal.GetControleReceptionStatuses().SingleOrDefault(p => p.Name == "ToBeValidated");
                    dal.InsertDeclarationControleReception(declaration);
                    result.Value = true;
                    result.IsSuccess = true;
                }
        ...
        [Route(nameof(UploadPictureCR))]
        [HttpPut]
        [ValidateMimeMultipartContentFilter]
        public async Task<PictureServiceResult> UploadPictureCR(string appVersion, string id, PictureType pictureType, string declarationId, string fileName)
        {
            var dal = new Dal();
            {
                ...
                {
                    var declaration = dal.FindDeclarationControleReception(declarationId);
                    if (declaration == null)
                    {
                ...
                if (fileData != null)
                {
                    ...
                    var samePicture = dal.GetPictures().SingleOrDefault(p => p.PicturePath == destFileName && p.DeclarationControleReception_Id == declarationId);
                    ...
                    var picture = dal.GetPictures().SingleOrDefault(p => p.PictureType == pictureType && p.DeclarationControleReception_Id == declarationId && p.CreatedBy.Id == id);
                    ...
                    picture = new Picture()
                    {
                        ...
                        DeclarationControleReception_Id = declarationId
                    };
        ...
        [Route(nameof(UploadPicCR))]
        [HttpPost]
        public async Task<PictureServiceResult> UploadPicCR(PictureParameter parameter)
        {
            var dal = new Dal();
            {
                var result = new PictureServiceResult();
                ...
                {
                    var declaration = dal.FindDeclarationControleReception(parameter.DeclarationId);
                    if (declaration == null)
                    {
                ...
                if (bytes.Length > 0)
                {
                    ...
                    name = name.Trim("\" ".ToCharArray());
                    var destFileName = Path.Combine(rootPath, name);
                    var samePicture = dal.GetPictures().SingleOrDefault(p => p.PicturePath == destFileName && p.DeclarationControleReception_Id == parameter.DeclarationId);
                    if (samePicture != null) //Already the same picture exists
                    {
                        var clone0 = samePicture.Clone() as Picture;
                        clone0.PicturePath = Url.Link("Default", new { controller = "Pictures", action = "GetPicture", id = samePicture.Id });
                    ...
                    var picture = dal.GetPictures().SingleOrDefault(p => p.PictureType == parameter.PictureType && p.DeclarationControleReception_Id == parameter.DeclarationId && p.CreatedBy.Id == parameter.Id);
                    if (picture != null) //Same user already has a picture
                    {
                        if (File.Exists(picture.PicturePath))
                            File.Delete(picture.PicturePath);
                    ...
                    picture = new Picture()
                              {
                                  ...
                                  PicturePath = destFileName,
                                  DeclarationControleReception_Id = parameter.DeclarationId
                              };
                    dal.InsertPicture(picture);
        ...
        [System.Web.Http.Route(nameof(GetAgencesCR))]
        public AgenceListServiceResult GetAgencesCR(string appVersion, string userId)
        {
            using (var dal = new Dal())
            {
                var user = dal.FindUser(userId);
                ...
                return new AgenceListServiceResult(dal.GetAgencesCR(userId).CloneList());
