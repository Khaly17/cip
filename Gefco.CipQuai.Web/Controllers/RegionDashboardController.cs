...
{
    [Authorize(Roles = "Super Admin,Admin,National Users,Regional Users,Web Users")]
    public class RegionDashboardController : ZoneController
    {
        public RegionDashboardController() : base(Zone.Regional)
        {
        }
        ...
        public ActionResult Index(FilterViewModel viewModel)
        {
            ...
            ViewBag.Agences = Ctx.TractionDefinitions.Where(q => q.AgenceDepart.Region_Id == viewModel.Id).GroupBy(p => p.AgenceDepart).Select(p => p.Key).CloneList();
            var user = Ctx.Users.Find(User.Identity.GetUserId());
            ViewBag.UserName = user.FirstName + " " + user.LastName;
            var region = Ctx.Regions.Single(p => p.Id == viewModel.Id);
        ...
        public ActionResult UtilisationDoublePlancher(FilterViewModel viewModel, string size, string title = null, string subTitle = "Utilisation double plancher", string centerLabel = "UTILISATION DP")
        {
            ...
            var region = Ctx.Regions.Find((string)viewModel.Id);
            ViewBag.Size = !size.IsNullOrWhiteSpace() ? size : "Medium";
            ViewBag.PieTitle = !title.IsNullOrWhiteSpace() ? title : region?.Name;
            ViewBag.SubTitle = subTitle;
            ViewBag.DeclarationType = "DP";
            ViewBag.EventType = "Region";
            ...
            if (size == "Medium")
                ViewBag.DetailsUrl = Url.Action("GrilleDP", new { viewModel.FilterType, viewModel.Id, startDate = viewModelStartDate.ToString("dd/MM/yyyy"), endDate = viewModelEndDate.ToString("dd/MM/yyyy") });
            else
                ViewBag.DetailsUrl = Url.Action("Index", "RegionDashboard", new { viewModel.Id, startDate = viewModelStartDate.ToString("dd/MM/yyyy"), endDate = viewModelEndDate.ToString("dd/MM/yyyy"), filterType = viewModel.FilterType });
        ...
        private StatDPViewModel GetDPData(string id, DateTime startDate, DateTime endDate)
        {
            ...
            model.DpNonUtilisées = Ctx.DeclarationDoublePlanchers.Count(p => p.Traction.AgenceDepart.Region_Id == id && p.Traction != null && !p.Traction.IsDeleted && !p.Traction.IsCancelled && !p.IsDeleted && p.Traction.DueDate < endDate && p.Traction.DueDate >= startDate && RemorqueStatus.DashboardStatuses.Contains(p.CurrentStatus.Name) && !p.IsDPUsed);
        ...
        [MyAuthorize(Roles = "Super Admin,Admin,National Users,Regional Users,Web Users", RegionRoles = "AQHSE,Alternant QHSE,RQHSE,RQCO", InputType = InputType.Region)]
        public ActionResult GrilleDP(FilterViewModel viewModel)
        {
            ...
            ViewBag.FilterType = viewModel.FilterType;
            var region = Ctx.Regions.Single(p => p.Id == viewModel.Id);
            ViewBag.PdfName = "Region - " + region.Name;
            ViewBag.Title = region.Name + " - Doubles planchers";
        ...
        public ActionResult GrilleDPA(FilterViewModel viewModel)
        {
            ...
            var endDate = viewModel.EndDate;
            endDate = endDate.AddDays(1);
            var definitions = Ctx.TractionDefinitions.Where(p => p.AgenceDepart.Region_Id == viewModel.Id).OrderBy(p => p.AgenceDepart.Region.Name).ThenBy(p => p.AgenceDepart.Name).ToList();
            var tractionsHorsDefinition = Ctx.Tractions.Where(p => p.DueDate < endDate && p.DueDate >= startDate && p.TractionDefinition == null && p.AgenceDepart.Region_Id == viewModel.Id).ToList();
            ...
            var tractions = Ctx.Tractions.Where(p => p.AgenceDepart.Region_Id == viewModel.Id && p.DueDate < endDate && p.DueDate >= startDate).ToList();
            var dpDeclarées = Ctx.DeclarationDoublePlanchers.Where(p => p.Traction != null && p.Traction.AgenceDepart.Region_Id == viewModel.Id && p.Traction.DueDate < endDate && p.Traction.DueDate >= startDate && !RemorqueStatus.DashboardStatuses.Contains(p.CurrentStatus.Name) || p.IsDeleted).ToList();
        ...
        public ActionResult ListeDP(FilterViewModel viewModel)
        {
            ...
            var startDate = viewModel.StartDate;
            var endDate = viewModel.EndDate.AddDays(1);
            var dpDeclarées = Ctx.DeclarationDoublePlanchers.Where(p => p.Traction != null && p.Traction.AgenceDepart.Region_Id == viewModel.Id && p.Traction.DueDate < endDate && p.Traction.DueDate >= startDate && RemorqueStatus.DashboardStatuses.Contains(p.CurrentStatus.Name)).ToList();
        ...
        {
            endDate = endDate.AddDays(1);
            var bp = Ctx.DeclarationBonnePratiques.Count(p => p.CreationDate < endDate && p.CreationDate >= startDate && p.Agence.Region_Id == id);
        ...
        public ActionResult ListeBP(FilterViewModel viewModel)
        {
            ...
            ViewBag.FilterType = viewModel.FilterType;
            var region = Ctx.Regions.Single(p => p.Id == viewModel.Id);
            ViewBag.Title = region.Name + " - Bonnes pratiques";
            var startDate = viewModel.StartDate;
            var endDate = viewModel.EndDate.AddDays(1);
            var bonnePratiques = Ctx.DeclarationBonnePratiques.Where(p => p.CreationDate != null && p.AutreAgentConcerné != null && p.Agence.Region_Id == viewModel.Id && p.CreationDate < endDate && p.CreationDate >= startDate).ToList();

            ViewBag.rUrl = Server.UrlEncode(Request.Url.PathAndQuery);

            var model = new ListeBPViewModel(viewModel, bonnePratiques.ToList());
            var rUrl = Server.UrlEncode(Request.Url.PathAndQuery);
            foreach (var item in model.Items)
            {
        ...
        {
            endDate = endDate.AddDays(1);
            var bp = Ctx.DeclarationSimplePlanchers.Count(p => p.CreationDate < endDate && p.CreationDate >= startDate && p.Agence.Region_Id == id && p.Traction_Id == null && p.AutreAgenceArrivee != null && !p.IsDeleted);
        ...
        public ActionResult ListeSP(FilterViewModel viewModel)
        {
            ...
            ViewBag.FilterType = viewModel.FilterType;
            var region = Ctx.Regions.Single(p => p.Id == viewModel.Id);
            ViewBag.Title = region.Name + " - Simples planchers";
            var startDate = viewModel.StartDate;
            var endDate = viewModel.EndDate.AddDays(1);
            var list = Ctx.DeclarationSimplePlanchers.Where(p => p.CreationDate < endDate && p.CreationDate >= startDate && p.Agence.Region_Id == viewModel.Id && p.Traction_Id == null && p.AutreAgenceArrivee != null && !p.IsDeleted).ToList();

            ViewBag.rUrl = Server.UrlEncode(Request.Url.PathAndQuery);

            var model = new ListeSPViewModel(viewModel, list.ToList());
            var rUrl = Server.UrlEncode(Request.Url.PathAndQuery);
            foreach (var item in model.Items)
            {
        ...
        {
            endDate = endDate.AddDays(1);
            var bp = Ctx.DeclarationControleReceptions.Count(p => p.CreationDate < endDate && p.CreationDate >= startDate && p.Agence.Region_Id == id && p.Traction_Id == null && p.AutreAgenceArrivee != null && !p.IsDeleted);
        ...
        public ActionResult ListeCR(FilterViewModel viewModel)
        {
            ...
            ViewBag.FilterType = viewModel.FilterType;
            var region = Ctx.Regions.Single(p => p.Id == viewModel.Id);
            ViewBag.Title = region.Name + " - Contrôles arrivage";
            var startDate = viewModel.StartDate;
            var endDate = viewModel.EndDate.AddDays(1);
            var list = Ctx.DeclarationControleReceptions.Where(p => p.CreationDate < endDate && p.CreationDate >= startDate && p.Agence.Region_Id == viewModel.Id && p.Traction_Id == null && p.AutreAgenceArrivee != null && !p.IsDeleted).ToList();

            ViewBag.rUrl = Server.UrlEncode(Request.Url.PathAndQuery);

            var model = new ListeCRViewModel(viewModel, list.ToList());
            var rUrl = Server.UrlEncode(Request.Url.PathAndQuery);
            foreach (var item in model.Items)
            {
        ...
        public ActionResult ListeNCRecues(FilterViewModel viewModel)
        {
            ...
            var ncs = Ctx.DeclarationNonConformites.Where(p => !p.IsDeleted && p.AgenceConcernée.Region_Id == viewModel.Id && p.CreationDate < endDate && p.CreationDate >= startDate);
        ...
        private StatNCViewModel GetNCRecuesData(string id, DateTime startDate, DateTime endDate)
        {
            ...
            var ncs = Ctx.DeclarationNonConformites.Where(p => p.AgenceConcernée.Region_Id == id && p.CreationDate < endDate && p.CreationDate >= startDate && !p.IsDeleted && p.MotifNCs.Any()).GroupBy(p => p.AgenceConcernée);
        ...
        public ActionResult ListeNCTypoRecues(FilterViewModel viewModel)
        {
            ...
            ViewBag.FilterType = viewModel.FilterType;
            var region = Ctx.Regions.Single(p => p.Id == viewModel.Id);
            ViewBag.Title = region.Name + " - Typologies de non conformités reçues";
        ...
        private StatNCViewModel GetNCTypoRecuesData(string id, DateTime startDate, DateTime endDate)
        {
            endDate = endDate.AddDays(1);
            var model = new StatNCViewModel();
            var items = Ctx.DeclarationNonConformites.Where(p => p.AgenceConcernée.Region_Id == id && p.CreationDate < endDate && p.CreationDate >= startDate && !p.IsDeleted && p.MotifNCs.Any()).ToList();
        ...
        public ActionResult ListeNCEmises(FilterViewModel viewModel)
        {
            ...
            var startDate = viewModel.StartDate.Date;
            var endDate = viewModel.EndDate.Date.AddDays(1);
            var ncs = Ctx.DeclarationNonConformites.Where(p => !p.IsDeleted && p.CreationDate < endDate && p.CreationDate >= startDate && p.Agence.Region_Id == viewModel.Id);
            var model = new ListeNCViewModel(viewModel, ncs.ToList());
            var rUrl = Server.UrlEncode(Request.Url.PathAndQuery);
            foreach (var item in model.Items)
            {
        ...
        private StatNCViewModel GetNCEmisesData(string id, DateTime startDate, DateTime endDate)
        {
            endDate = endDate.AddDays(1);
            var model = new StatNCViewModel();
            var ncs = Ctx.DeclarationNonConformites.Where(p => p.CreationDate < endDate && p.CreationDate >= startDate && !p.IsDeleted && p.Agence.Region_Id == id && p.MotifNCs.Any()).GroupBy(p => p.Agence);
            model.Data = ncs.ToList().Select(p => new DataValue(p.Key.Name, p.Count())).OrderByDescending(p => p.Value).Take(3).ToList();
            return model;
        }
        ...
        public ActionResult ListeNCTypoEmises(FilterViewModel viewModel)
        {
            ...
            ViewBag.FilterType = viewModel.FilterType;
            var region = Ctx.Regions.Single(p => p.Id == viewModel.Id);
            ViewBag.Title = region.Name + " - Typologies de non conformités émises";
        ...
        private StatNCViewModel GetNCTypoEmisesData(string id, DateTime startDate, DateTime endDate)
        {
            endDate = endDate.AddDays(1);
            var model = new StatNCViewModel();
            var items = Ctx.DeclarationNonConformites.Where(p => p.Agence.Region_Id == id && p.CreationDate < endDate && p.CreationDate >= startDate && !p.IsDeleted && p.MotifNCs.Any()).ToList();
            var ncs = items.SelectMany(p => p.MotifNCs).Select(p => p.Name).ToList();
            model.Data = ncs.GroupBy(p => p).Select(p => new DataValue(p.Key, p.Count())).OrderByDescending(p => p.Value).Take(3).ToList();
            return model;
        }
